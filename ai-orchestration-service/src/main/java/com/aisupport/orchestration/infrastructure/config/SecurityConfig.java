package com.aisupport.orchestration.infrastructure.config;
    ...
    SecurityFilterChain securityFilterChain(
        ...
        try {
	        http
	            ...
	            // reliably slot the CookieGuardFilter in front of it.
	            .addFilterBefore(headerAuthFilter, UsernamePasswordAuthenticationFilter.class)
	            
	            // As per ADR-004, CookieGuardFilter intercepts requests BEFORE any 
	            // application code (including auth parsing) touches them, blocking
	            // implicit credential usage except on explicitly allowed paths.
	            .addFilterBefore(cookieGuardFilter, HeaderAuthenticationFilter.class);
	
	        return http.build();
        } catch (Exception e) {
            throw new SecurityConfigurationException("Failed to configure security filter chain", e);
        }
    }
}
