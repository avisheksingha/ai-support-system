...
- **Spring Security Filter Registration Gotcha**: When using `.addFilterBefore(filterA, filterClassB)`, Spring relies on an internal `FilterOrderRegistration` comparator. Standard filters (like `UsernamePasswordAuthenticationFilter`) have preset orders. Custom filters (like `HeaderAuthenticationFilter`) do not have an assigned order until they are registered relative to a standard filter. Therefore, attempting to register `CookieGuardFilter` before `HeaderAuthenticationFilter` will throw an `IllegalArgumentException` unless `HeaderAuthenticationFilter` is successfully registered *first*. The correct sequence anchors the custom auth filter to a standard filter, and then anchors the guard filter to the newly registered custom auth filter.
